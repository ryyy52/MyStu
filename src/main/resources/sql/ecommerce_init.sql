...
('devuser', MD5('dev123456'), 'dev@local.test', '13800000000', 'Beijing Road 1', 1),
('alice', MD5('alice123'), 'alice@example.com', '13900000000', 'Shenzhen Road 2', 1);

INSERT INTO category (name, parent_id, level, sort, icon, description) VALUES
('电子产品', 0, 1, 0, NULL, NULL),
('服饰鞋包', 0, 1, 0, NULL, NULL);

SET @cat_elec := (SELECT id FROM category WHERE name='电子产品');
SET @cat_wear := (SELECT id FROM category WHERE name='服饰鞋包');
...
SET @user_id := (SELECT id FROM user WHERE username='devuser');

INSERT INTO `order` (order_no, user_id, total_amount, status, receiver_name, receiver_phone, receiver_address)
VALUES ('TESTORDER0001', @user_id, 0.00, 1, '张三', '13600000000', '上海市黄浦区中山东一路1号');

SET @order_id := LAST_INSERT_ID();
SET @pA := (SELECT id FROM product WHERE name='测试商品A');
SET @pB := (SELECT id FROM product WHERE name='测试商品B');
...
-- 扩充用户与分类层级
INSERT INTO user (username, password, email, phone, address, status) VALUES
('bob', MD5('bobpass123'), 'bob@example.com', '13700000000', 'Guangzhou Road 3', 1),
('carol', MD5('carolpass456'), 'carol@example.com', '13600000001', 'Hangzhou Road 4', 1);
...
SET @cat_phone := (SELECT id FROM category WHERE name='手机');
SET @cat_laptop := (SELECT id FROM category WHERE name='笔记本');
SET @cat_men := (SELECT id FROM category WHERE name='男装');
SET @cat_women := (SELECT id FROM category WHERE name='女装');
...
SET @dev_id := (SELECT id FROM user WHERE username='devuser');
SET @alice_id := (SELECT id FROM user WHERE username='alice');

INSERT INTO cart (user_id) VALUES (@dev_id);
INSERT INTO cart (user_id) VALUES (@alice_id);
...
SET @pPhoneX := (SELECT id FROM product WHERE name='旗舰手机X');
SET @pLaptopPro := (SELECT id FROM product WHERE name='轻薄本Pro');
SET @pMenT := (SELECT id FROM product WHERE name='纯色T恤');
...
INSERT INTO `order` (order_no, user_id, total_amount, status, receiver_name, receiver_phone, receiver_address)
VALUES ('TESTORDER0002', @dev_id, 0.00, 1, '李四', '13500000000', '北京市海淀区学院路5号');

SET @order2 := LAST_INSERT_ID();
SET @pFlagship := (SELECT id FROM product WHERE name='旗舰手机X');
SET @pTshirt := (SELECT id FROM product WHERE name='纯色T恤');
...
-- 为 alice 创建一个已支付订单
INSERT INTO `order` (order_no, user_id, total_amount, status, receiver_name, receiver_phone, receiver_address)
VALUES ('ALICEORDER0001', @alice_id, 0.00, 2, '王五', '13400000000', '广州市天河区体育东路6号');

SET @orderAlice := LAST_INSERT_ID();
SET @pAir := (SELECT id FROM product WHERE name='商务本Air');
SET @pDress := (SELECT id FROM product WHERE name='连衣裙');
